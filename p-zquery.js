...
function json2url(json){
    var arr=[];
    for(var name in json){
        arr.push(name+'='+encodeURLComponent(json[name]));
    }
    return arr.join('&');
}
function ajax(json){
    ...
    switch(json.type.toLowerCase()){
        case 'get':
            oAjax.open('GET',json.url+json2ur(josn.data),true);
            oAjax.send();
            break;
        case 'post':
...
function move(obj,json,options){
    ...
    options = option ||{};
    option.easing = options.easing || 'ease-out';
